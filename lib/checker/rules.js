/**
 * rules.js — οι 46 κανόνες του ελέγχου.
 *
...
// 1.2.1 — το probe αντίθεσης έγραφε το αποτέλεσμα έξω από το <body> και ο
//          scraper της Cloudflare δεν το έβρισκε· ο A11Y-06 έβγαινε «blocked».
// 1.2.0 — προστέθηκε ο A11Y-06 (αντίθεση κειμένου WCAG AA), η πρώτη μέτρηση
//          που γίνεται σε πραγματικό browser. Βγαίνει `na` όταν το Browser
//          Rendering δεν είναι διαθέσιμο, ώστε να μην αλλάζει βαθμούς τυχαία.
// 1.1.1 — ο TRUST-02 δεν έβλεπε το «Γ.Ε.ΜΗ.» (τελεία ανάμεσα σε κάθε γράμμα)
//          και οι διευθύνσεις σε greeklish με w (epikoinwnia) έμεναν αόρατες.
// 1.1.0 — προστέθηκε ο SEO-10 (εικονίδιο σελίδας).
// Ο βαθμός ενός site αλλάζει όταν αλλάζουν οι κανόνες, οπότε η έκδοση μπαίνει
// σε κάθε αναφορά: δύο reports με διαφορετική έκδοση δεν συγκρίνονται.
...
/** true όταν ο ελεγχόμενος στόχος είναι το ίδιο hostname με τον ελεγκτή. */
const isSelfCheck = (ctx) => {
  if (!ctx.selfHost) return false;
  try {
    return new URL(ctx.url).host.toLowerCase() === String(ctx.selfHost).toLowerCase();
  } catch {
    return false;
  }
};
    ...
    run(ctx) {
      ...
      if (!hop) {
        if (probe.finalIsHttps) {
          return { state: PASS, measured: "Η σύνδεση γίνεται ήδη μέσω HTTPS." };
        }
        // Όταν ο στόχος είναι το hostname στο οποίο τρέχει ο ίδιος ο ελεγκτής,
        // το αίτημά μας δεν φεύγει ποτέ ως http: η πλατφόρμα αναβαθμίζει σιωπηλά
        // τη σύνδεση πριν φύγει. Μετρήθηκε: το middleware μας δέχεται το αίτημα
        // με url.protocol === "https:" ενώ εμείς ζητήσαμε http://. Δεν βλέπουμε
        // λοιπόν «απουσία ανακατεύθυνσης» — δεν υπήρξε καν http αίτημα να
        // ανακατευθυνθεί. Δεν βαθμολογούμε μέτρηση που δεν έγινε (ίδια λογική με
        // PERF-02/PERF-05). Ο κανόνας ενεργοποιείται μόνο σε αυτοέλεγχο· κάθε
        // άλλο site ελέγχεται κανονικά.
        if (isSelfCheck(ctx)) {
          return {
            state: NA,
            measured:
              "Ο έλεγχος δεν μπορεί να ζητήσει http από το ίδιο του το hostname — η πλατφόρμα αναβαθμίζει αυτόματα τη σύνδεση σε https. Δεν υπάρχει μέτρηση, οπότε ο κανόνας δεν μετράει στη βαθμολογία. Ελέγξτε το με: curl -sI http://…",
          };
        }
        return { state: FAIL, measured: "Το http:// απαντά κανονικά χωρίς ανακατεύθυνση σε https." };
      }
      if (!probe.finalIsHttps) {
...
const SEO_RULES = [
  ...
  },
  {
    ...
    run(ctx) {
      ...
      // Οι διαστάσεις είναι προαιρετικές: χωρίς αυτές το Facebook κατεβάζει
      // πρώτα την εικόνα για να τις βρει, και η πρώτη κοινοποίηση βγαίνει
      // χωρίς προεπισκόπηση. Δεν κοστίζει βαθμούς — είναι συμβουλή, όχι λάθος.
      const dims = og.image && !(og["image:width"] && og["image:height"])
        ? "Λείπουν τα og:image:width / og:image:height. Χωρίς αυτά η προεπισκόπηση αργεί να εμφανιστεί την πρώτη φορά που μοιράζεται ο σύνδεσμος."
        : undefined;

      if (missing.length === 0) return { state: PASS, measured: "Και τα τέσσερα υπάρχουν.", note: dims };
      if (missing.length === required.length) return { state: FAIL, measured: "Δεν υπάρχει καμία ετικέτα Open Graph." };
      return { state: PARTIAL, measured: `Λείπουν: ${missing.map((m) => `og:${m}`).join(", ")}.`, note: dims };
    },
  },
  {
    id: "SEO-09",
  ...
  {
    id: "SEO-10",
    title: "Εικονίδιο σελίδας (favicon & apple-touch-icon)",
    units: 1,
    meaning:
      "Το εικονίδιο είναι ό,τι ξεχωρίζει την καρτέλα σας από τις άλλες είκοσι και ό,τι μένει στην αρχική οθόνη του κινητού όταν κάποιος προσθέσει το site.",
    fix: "Προσθέστε <link rel=\"icon\" href=\"/favicon.svg\"> και <link rel=\"apple-touch-icon\" href=\"/apple-touch-icon.png\"> (180×180) στο <head>.",
    run(ctx) {
      const { icon, appleTouch } = ctx.facts.icons;

      // Οι browsers ζητούν από μόνοι τους το /favicon.ico ακόμη κι όταν δεν
      // δηλώνεται πουθενά. Το να χρεώναμε αποτυχία χωρίς να το κοιτάξουμε θα
      // τιμωρούσε site που εμφανίζουν κανονικότατα εικονίδιο.
      const probe = ctx.probes.favicon;
      const type = probe?.headers?.get("content-type") || "";
      const implicit = Boolean(probe?.ok) && probe.status < 400 && !/text\/html/i.test(type);
      const hasIcon = icon || implicit;

      if (hasIcon && appleTouch) {
        return {
          state: PASS,
          measured: icon
            ? "Δηλώνονται και favicon και apple-touch-icon."
            : "Δηλώνεται apple-touch-icon και το /favicon.ico εξυπηρετείται κανονικά.",
        };
      }
      ...
      if (hasIcon) {
        return {
          state: PARTIAL,
          measured: icon
            ? "Δηλώνεται favicon, αλλά όχι apple-touch-icon."
            : `Δεν δηλώνεται <link rel="icon">, αλλά το /favicon.ico απαντά ${probe.status} (${type.split(";")[0] || "χωρίς τύπο"}).`,
          note: "Στο iOS, «Προσθήκη στην αρχική οθόνη» χωρίς apple-touch-icon βάζει ένα στιγμιότυπο της σελίδας αντί για λογότυπο.",
        };
      }

      // Δεν δηλώνεται τίποτα και ο δικός μας έλεγχος του /favicon.ico δεν
      // απάντησε: δεν ξέρουμε, άρα δεν βαθμολογούμε. Ένα δικό μας timeout δεν
      // είναι σφάλμα του site.
      if (!probe || !probe.ok) {
        return { state: NA, measured: "Δεν δηλώνεται εικονίδιο και το /favicon.ico δεν μπόρεσε να ελεγχθεί." };
      }
      return {
        state: FAIL,
        measured:
          probe.status >= 400
            ? `Κανένα εικονίδιο: δεν δηλώνεται <link rel="icon"> και το /favicon.ico απαντά ${probe.status}.`
            : `Κανένα εικονίδιο: το /favicon.ico απαντά ${probe.status} με ${type.split(";")[0] || "άγνωστο τύπο"} αντί για εικόνα.`,
      };
    },
  },
];
...
const A11Y_RULES = [
  ...
  {
    id: "A11Y-06",
    title: "Αντίθεση κειμένου (WCAG AA)",
    units: 3,
    meaning: "Κείμενο με χαμηλή αντίθεση από το φόντο του δεν διαβάζεται από άτομα με μειωμένη όραση, ούτε από κανέναν σε κινητό στον ήλιο. Το όριο ΑΑ είναι 4.5:1 για κανονικό κείμενο και 3:1 για μεγάλο (24px ή 18.66px έντονο).",
    fix: "Σκουρύνετε το χρώμα του κειμένου ή ανοίξτε το φόντο μέχρι η αναλογία να φτάσει το 4.5:1. Συνήθως φταίνε τα «ανοιχτά γκρι» δευτερεύοντα κείμενα και τα ανοιχτά χρώματα πάνω σε λευκό.",
    run(ctx) {
      const c = ctx.contrast;
      if (!c || !c.available) {
        const why = {
          quota: "εξαντλήθηκε ο ημερήσιος χρόνος του browser",
          timeout: "η σελίδα δεν φόρτωσε εγκαίρως στον browser",
          blocked: "η πολιτική ασφαλείας της σελίδας (CSP) δεν άφησε τη μέτρηση να τρέξει",
          unconfigured: "η μέτρηση σε browser δεν είναι ενεργή",
        }[c?.reason] || "ο browser δεν επέστρεψε αποτέλεσμα";
        return { state: NA, measured: `Δεν μετρήθηκε — ${why}.` };
      }
      if (!c.measured) return { state: NA, measured: "Δεν βρέθηκε μετρήσιμο κείμενο." };

      const skipped = c.unmeasured ? ` Δεν μετρήθηκαν ${c.unmeasured} πάνω σε εικόνα ή gradient.` : "";
      if (c.failing === 0) {
        return { state: PASS, measured: `${c.measured} κείμενα, όλα πάνω από το όριο ΑΑ.${skipped}` };
      }
      const example = c.examples?.[0];
      const sample = example ? ` Π.χ. «${example.t}» (${example.s}) στο ${example.r}:1 αντί ${example.n}:1.` : "";
      const measured = `${c.failing} από ${c.measured} κείμενα κάτω από το όριο ΑΑ.${sample}${skipped}`;
      const evidence = (c.examples || []).map((e) => `${e.s}: «${e.t}» ${e.r}:1 (όριο ${e.n}:1)`);
      return { state: c.failing <= 5 ? PARTIAL : FAIL, measured, evidence };
    },
  },
];
...
const TRUST_RULES = [
  {
    ...
    run(ctx) {
      ...
      const hasContactPage = trustHrefs(ctx).some((href) =>
        // «epikoin» αντί για «epikoinon»: στα greeklish το ω γίνεται και w
        // (epikoinwnia) — η παλιά έκφραση έχανε τη μισή Ελλάδα.
        /epikoin|epikin|contact|kontakt|επικοινων/i.test(decodeHref(href))
      );
  ...
  },
  {
    ...
    run(ctx) {
      // Οι συντομογραφίες γράφονται με κάθε δυνατό συνδυασμό τελειών: ΓΕΜΗ,
      // ΓΕ.ΜΗ., Γ.Ε.ΜΗ., Γ.Ε.Μ.Η. — γι' αυτό η τελεία (και το κενό) είναι
      // προαιρετική ΑΝΑΜΕΣΑ ΣΕ ΚΑΘΕ γράμμα αντί σε μία συγκεκριμένη θέση.
      // Οι κλάσεις [ΑA] [ΕE] [ΜM] [ΗH] πιάνουν και τα λατινικά ομοιογράμματα:
      // σε footer γραμμένο με copy-paste το «Α.Φ.Μ.» συχνά κρύβει λατινικό A ή M
      // και δεν φαίνεται με το μάτι πουθενά.
      const afmRe = /[ΑA]\.?\s?Φ\.?\s?[ΜM]\.?\s*:?\s*(?:EL|GR)?\s*\d{9}|\bVAT\b[^\d]{0,12}\d{9}/i;
      const gemiRe = /Γ\.?\s?[ΕE]\.?\s?[ΜM]\.?\s?[ΗH]\.?\s*:?\s*\d{6,}|\bGEM[IH]\b[^\d]{0,12}\d{6,}/i;
  ...
  },
  {
    ...
    run(ctx) {
      const found = trustHrefs(ctx).some((href) =>
        // Το «prostasi…-dedomen…» θέλει ΚΑΙ τις δύο λέξεις: σκέτο «dedomenon»
        // θα έπιανε και το /vasi-dedomenon/ μιας εταιρείας λογισμικού.
        /privacy|aporrit|απορρ|prosopik|gdpr|cookies?-?polic|politiki-cookies|prostasi[a-z]*[-_ ]?dedomen|προστασ[^/]{0,14}δεδομ/i.test(
          decodeHref(href)
        )
      );
