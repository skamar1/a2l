  ...
  },
  {
    ...
    run(ctx) {
      ...
      if (!encoding) {
        return {
          state: FAIL,
          measured: "Δεν δηλώνεται content-encoding — το HTML στέλνεται ασυμπίεστο.",
        };
      }
...
const TRUST_RULES = [
  {
    ...
    run(ctx) {
      ...
      const hasContactPage = ctx.facts.links.hrefs.some((href) =>
        /epikoinon|contact|επικοινων/i.test(href)
      );
  ...
  },
  {
    ...
    run(ctx) {
      const text = ctx.facts.text;
      const hasAfm = /(Α\.?Φ\.?Μ\.?|ΑΦΜ|VAT)\s*:?\s*(EL)?\s*\d{9}/i.test(text);
      const hasGemi = /(ΓΕ\.?ΜΗ\.?|ΓΕΜΗ)\s*:?\s*\d{6,}/i.test(text);
      if (hasAfm && hasGemi) return { state: PASS, measured: "Βρέθηκαν ΑΦΜ και ΓΕΜΗ." };
      if (hasAfm || hasGemi) return { state: PARTIAL, measured: `Βρέθηκε ${hasAfm ? "ΑΦΜ" : "ΓΕΜΗ"}, λείπει το άλλο.` };
      return { state: FAIL, measured: "Δεν βρέθηκε ΑΦΜ ούτε ΓΕΜΗ στη σελίδα." };
    },
  },
  {
    id: "TRUST-03",
    ...
    run(ctx) {
      const found = ctx.facts.links.hrefs.some((href) =>
        /privacy|aporrit|απορρ|prosopik|gdpr|cookies?-?polic|politiki-cookies/i.test(href)
      );
